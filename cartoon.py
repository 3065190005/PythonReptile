...
if (len(os.sys.argv) == 2):
    if(os.sys.argv[1] == "-h"):
        print("-di [Url]    : get imomoe video mp4 url")
        exit(0)
...
if (len(os.sys.argv) == 3):
    if(os.sys.argv[1] == "-di"):
        ...
        videoEpic = os.sys.argv[2]
        videoEpic = videoEpic.replace("http://www.imomoe.in/player/",'')
        videoEpic = videoEpic.replace('.html','')
        videoEpic = videoEpic[videoEpic.find('-') + 1:]
        videoEpic = videoEpic[videoEpic.find('-') + 1:]

        videoAddress = os.sys.argv[2]
        videoAddress = videoAddress.replace("http://www.imomoe.in/player/",'')
        videoAddress = videoAddress.replace('.html','')
        videoAddress = videoAddress[videoAddress.find('-')+1:]
        videoAddress = videoAddress.replace(videoEpic,'')
        videoAddress = videoAddress.replace('-', '')

        while 1:
            print("正在连接下载链接 第",post+1,"次")
            r = requests.get(os.sys.argv[2])
            r.encoding = 'gb2312'
            ...
            if(post >= 10):
                ...
                print("解析失败\n")
                exit(-1)
        dmp4 = re.search('src="/playdata/(.*?)\"></script>',r.text).group()
        dmp4 = dmp4.replace('src="','')
        dmp4 = dmp4.replace('\"></script>','')
        dmp4 = "http://www.imomoe.in"+dmp4
        if(dmp4 != None):
            jsond = requests.get(dmp4)
            jsond.encoding='gb2312'
            downloadurl = re.findall('\$https://gss3.baidu.com/(.*?).mp4\$',jsond.text)
            print("解析下载成功\n")
            for i in range(len(downloadurl)):
                downloadurl[i] = 'https://gss3.baidu.com/'+downloadurl[i]+'.mp4'
            print("第 ",int(videoEpic)+1," 集下载MP4")
            print(downloadurl[int(videoEpic)])
        else:
            print("解析下载失败\n")
        exit(0)
...
search = input("pls type main word\n")
